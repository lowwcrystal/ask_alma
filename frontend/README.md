...
- **RAG System**: 
  ...
  - Context retrieval from course bulletins and CULPA reviews
...
During development, we encountered several technical challenges that required careful problem-solving and iteration.
...
**Impact**: This optimization significantly improved the relevance of retrieved context, leading to more accurate and complete answers.
...
- Course bulletins were in PDF format requiring extraction
- CULPA reviews needed web scraping with proper rate limiting
...
2. **Web Scraping**:
   - Implemented respectful scraping with rate limiting
   - Handled dynamic content and pagination
   - Built robust error handling for network issues

3. **Data Cleaning**:
   - Created scripts to normalize course codes and names
   - Removed duplicates and merged related information
   - Validated data integrity before embedding

4. **Data Updates**:
   - Designed modular data processing pipeline
   - Created scripts to incrementally update embeddings
   - Implemented versioning for data sources
...
**Key Learnings**:
- Personalization requires coordination between database queries, prompt design, and LLM instructions
- Testing with diverse user profiles is essential
- Balance between specificity and flexibility is crucial
