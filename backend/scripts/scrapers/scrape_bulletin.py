#!/usr/bin/env python3
"""
Script to recursively scrape Columbia Engineering bulletin pages
...
def scrape_page(url, visited_urls):
    ...
    try:
        ...
        for link in soup.find_all('a', href=True):
            ...
            # Only include links that are within the same domain and catalog/bulletin section
            parsed_link = urlparse(full_url)
            if parsed_link.netloc == base_domain:
                # Check if it's a relevant page
                # For Columbia Engineering: contains 'bulletin.columbia.edu' and 'columbia-engineering'
                # For Barnard: contains 'catalog.barnard.edu' and 'barnard-college'
                if (('bulletin.columbia.edu' in full_url and 'columbia-engineering' in full_url) or
                    ('catalog.barnard.edu' in full_url and 'barnard-college' in full_url)):
                    links.append({
                        'text': link_text,
                        'url': full_url
                    })
...
def scrape_bulletin_recursive(start_url, output_file='seas_2026.json', max_pages=1000):
    """
    Recursively scrape all Columbia Engineering bulletin pages
    ...
    with open(txt_file, 'w', encoding='utf-8') as f:
        f.write(f"Columbia Engineering Bulletin Scrape Summary\n")
        f.write(f"{'='*80}\n\n")
...
if __name__ == "__main__":
    # Starting URL
    url = "https://bulletin.columbia.edu/columbia-engineering/about-school/"
    
    # Recursively scrape all pages
    scrape_bulletin_recursive(url, output_file='seas_2026.json', max_pages=1000)
