#!/usr/bin/env python3
"""
Script to recursively scrape Barnard College catalog pages
...
def scrape_page(url, visited_urls):
    """
    Scrape a single catalog page and extract all content
    ...
    try:
        ...
        for link in soup.find_all('a', href=True):
            ...
            # Only include links that are within the same domain and catalog section
            parsed_link = urlparse(full_url)
            if parsed_link.netloc == base_domain:
                # Check if it's a Barnard catalog page
                if 'catalog.barnard.edu' in full_url and 'barnard-college' in full_url:
                    links.append({
                        'text': link_text,
                        'url': full_url
                    })
...
def scrape_catalog_recursive(start_url, output_file='barnard_2026.json', max_pages=1000):
    """
    Recursively scrape all Barnard College catalog pages
    ...
    with open(txt_file, 'w', encoding='utf-8') as f:
        f.write(f"Barnard College Catalog Scrape Summary\n")
        f.write(f"{'='*80}\n\n")
...
if __name__ == "__main__":
    # Starting URL
    url = "https://catalog.barnard.edu/barnard-college/message-president/"
    
    # Recursively scrape all pages
    scrape_catalog_recursive(url, output_file='barnard_2026.json', max_pages=1000)
